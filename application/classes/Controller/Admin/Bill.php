...
class Controller_Admin_Bill extends Controller_Admin_Layout
{
	...
	{
       if ($total = ORM::factory('bill')->where('status', 'IN', array(5,6))->count_all())
		{
			$per_page = 12;
            $page = $this->request->param('page', 1);
			$paging = Paging::make($total, $page, 10, 4, 'admin/bill');
			$bills = ORM::factory('bill')->where('status', '=', 5)->order_by('created', 'desc')
				->offset(($page - 1) * $per_page)->limit($per_page)->find_all();
                
    	}
        
        $filters = Model_Filter::instance('bill', array('filters' => array('id', 'created', 'active'),
                                          'orders' => array('id', 'created', 'active'))
                                          );
                                 
		$this->template->content = View::factory('admin/bill/list')->bind('bills', $bills)->bind('paging', $paging);
	}
	...
	{
		$bill = ORM::factory('bill', $this->request->param('id'));
		if ( ! $bill->loaded()){
			throw new HTTP_Exception_404();
		}
		$bill->delete();
		...
		else
		{
            ...
            Controller::redirect('admin/bill');
		}
	}
